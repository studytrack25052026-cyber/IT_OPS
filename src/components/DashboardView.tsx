...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const myRequests = changeRequests.filter((cr) => cr.requesterId === currentUser.id);
                ...
                <tr>
                  <td colSpan={6} className="p-10 text-center text-slate-500">
                    <div className="flex flex-col items-center justify-center space-y-2.5">
                      ...
                      <p className="font-bold text-sm text-slate-800">No Change Requests Found in Database</p>
                      <p className="text-xs text-slate-500 max-w-md">
                        There are currently no active tickets in the PostgreSQL database. Use the button below to submit your first change request or service ticket.
                      </p>
                      <button
                        onClick={onCreateNewRequest}
                        className="mt-2 inline-flex items-center space-x-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold shadow-sm transition-all"
                      >
